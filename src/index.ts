...
import { LoggingService, LoggingServiceConfig, LogLevel } from './services/logging-service';
...
const config = {
  ...
  eufy: {
    ...
    persistentDir: path.join(__dirname, process.env.PERSISTENT_DIR || 'persistent'),
    ...
    pollingIntervalMinutes: parseInt(process.env.POLLING_INTERVAL_MINUTES || '10'),
  ...
  mqtt: {
    ...
    clientId: process.env.MQTT_CLIENT_ID || `eufy-mqtt-${Math.random().toString(16).substring(2, 8)}`,
...
if (!config.eufy.username || !config.eufy.password) {
  loggingService.error('Error: EUFY_USERNAME and EUFY_PASSWORD environment variables are required');
  process.exit(1);
}
...
async function main() {
  try {
    ...
    eufyService = new EufyService(config.eufy, loggingService, config.logging.level as LogLevel);
    ...
    webSocketService = new WebSocketService(server, eufyService, loggingService);
